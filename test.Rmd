...
library(spdep)
negative_bino = glm.nb(living_with ~ as.factor(ethnicity) + as.factor(educ_level) +
    as.factor(marital_status) + as.factor(drug) + as.factor(shared_needles) +
    as.factor(sexual_orientation) + age + household_income +
    sex_partners, data = all_with_living_with)
#summary(negative_bino)
#pchisq( 2 * (logLik(negative_bino) - logLik(final_poisson) ), df = 1 ,lower.tail =  ALSE)
print(test.nb.pois(negative_bino,final_poisson))
print(DeanB(final_poisson))
print(DeanB2(final_poisson))
```
```{r}
summary(negative_bino)
```
