...
```{r}
new_model <- glm(hivstat ~ as.factor(ethnicity) + as.factor(drug) + as.factor(ever_sex_with_hiv_male) + as.factor(household_income) + as.factor(sex_partners) + as.factor(ever_jail), data = all, family = binomial())

summary(new_model)
```
We notice that the coefficients of the number of sex partners, ever being in jail, and ever having sex with an hiv positive male are very statistically insignificant as the p-values are almost equal to 1. 
...
We fail to reject the null hypothesis that the model provides a good fit (p-value = 0.5509 > 0.05) and conclude that our model provides a good fit of the data. 
...
Checking if adding age provides a better fit than the model without age.

```{r}

new_modelage <- new_model <- glm(hivstat ~ as.factor(ethnicity) + as.factor(drug) + as.factor(ever_sex_with_hiv_male) + as.factor(household_income) + as.factor(sex_partners) + as.factor(ever_jail), data = all, family = binomial())
