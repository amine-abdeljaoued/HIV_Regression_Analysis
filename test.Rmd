...
```
The model that we get using forward selection includes too many covariates. In our research we prefer simpler models, so we are going to stick to the backward model instead of the forward one. However, we are going to add drug injection since it is a confounder.
...
```
The AIC of the model without the outliers is so much lower than the AIC of the model including the outliers. 
