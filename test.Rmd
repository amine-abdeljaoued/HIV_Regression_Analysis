...
exp(0.05201*10)
```
For every 10 year increas in age, the model predicts the odds of death to increase by a factor of 1.68
```{r}
hist1 = hist(all$age[all$death==1], plot = F, freq = F)
hist2 = hist(all$age[all$death==0], plot = F, freq = F)
plot( hist1, col=rgb(1,0,0,1/3), xlim=c(15,70), ylim = c(0,250))  # first histogram
...
```
As we can see in the plot of the histogram of women who died(red) and the histogram of the women who died, the women who died were on average older than the women who did not die.