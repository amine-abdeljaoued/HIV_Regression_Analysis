...
Because the effect of household income is statistically highly significant with a p value < 0.001 we also looked if we could improve the model by includeing quadratic household income or treating household income as a categorical variable.
...
```{r}
plot(all_with_CD4N$household_income,backwardModel$residuals)
```
```{r}
hist(backwardModel_CD4$fitted.values)
hist(all_with_CD4N$CD4N_t)
plot(all_with_CD4N$CD4N_t,backwardModel_CD4$fitted.values)
```
