...
```
```{r}
start_cd4 = lm(CD4N_t ~ as.factor(ethnicity) + as.factor(educ_level) + as.factor(marital_status) + living_with + household_income + as.factor(drug) + sex_partners + as.factor(ever_marijuana) + as.factor(shared_needles) + as.factor(ever_jail) + as.factor(ever_sex_with_hiv_male) + as.factor(ever_sex_with_gay_male) + as.factor(condom_use) + as.factor(ever_anal_sex) + as.factor(ever_sex_toys) + as.factor(sexual_orientation) + age , data = all_with_CD4N)
summary(start_cd4)
```
```{r}
...
```
The model selected by backward selection includes the categorical variables marital status, ever marijuana use, ever shared needles, ever in jal, ever sex with a hiv positive male, ever used condoms and ever used sex toys. It also includes household income which is treated as a continious variable because it has 8 categories. 1 being the lowest household income of 500$ or less and 8 being the highest with a household income of over 6250$.

Because the effect of household income is statistically highly significant with a p value < 0.001 we also looked if we could improve the model by including quadratic household income or treating household income as a categorical variable.

```{r}
all_with_CD4N$household_income2 = all_with_CD4N$household_income**2
backwardModel_CD4_modified = lm(CD4N_t ~ as.factor(marital_status) + household_income + household_income2+ as.factor(ever_marijuana) + as.factor(shared_needles) + as.factor(ever_jail) + as.factor(ever_sex_with_hiv_male) + as.factor(condom_use) + as.factor(ever_sex_toys), data = all_with_CD4N)
backwardModel_CD4_modified_2 = lm(CD4N_t ~ as.factor(marital_status) + as.factor(household_income) + as.factor(ever_marijuana) + as.factor(shared_needles) + as.factor(ever_jail) + as.factor(ever_sex_with_hiv_male) + as.factor(condom_use) + as.factor(ever_sex_toys), data = all_with_CD4N)
...
From our conversation with Dr. Caitlin Dugdale, infectious disease physician with expertise in HIV research, we know that she does not think that use of sex toys would be a strong predictor.
Furthermore, the two terms for the categorical variable ever_sex_toys are both not statistically significant at the alpha = 0.05 level.
We therefore decided to exclude this variable from our model
```{r}
#all_with_CD4N$condom_use = relevel(condom_use, ref = 1)
backwardModel_CD4_modified_3 = lm(CD4N_t ~ as.factor(marital_status) + household_income + as.factor(ever_marijuana) + as.factor(shared_needles) + as.factor(ever_jail) + as.factor(ever_sex_with_hiv_male) + as.factor(condom_use) , data = all_with_CD4N)
anova(backwardModel_CD4_modified_3, backwardModel_CD4)
```
The Anova also does not show that the model including the predictor variable ever_sex_toys statistically significantly improves the model at the alpha = 0.05 level.
We therefore select this model as our final model
```{r}
backwardModel_CD4_final = backwardModel_CD4_modified_3
```
...
We see that a one unit increase in household income is associated with an estimated 0.425(95% CI [0.205, 0.645]) decrease in the square root of the number of CD4 cells per cubic millimeter, holding all other variables in the model constant.
For marital status we see that being widowed is associated with an estimated 2.71(95% CI [0.88, 4.54]) decrease in the square root of the number of CD4 cells per cubic millimeter compared to being married, holding all other variables constant.
We also see that being divorced is associated with an estimated 2.19(95% CI [0.52, 3.86]) decrease in the square root of the number of CD4 cells per cubic millimeter compared to being married, holding all other variables constant.

For condom use, we see that never using condoms is associated with an estimated 3.50(95% CI [2.01, 4.99]) increase in the square root of the number of CD4 cells per cubic millimeter compared to always using condoms, holding all other variables constant.