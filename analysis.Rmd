...
```{r}
c_distance = cooks.distance(backwardModel_CD4_final)
n = length(c_distance)
c_distance[c_distance>12/n]
...
Same graph as above but hiv stat color coded:
```{r}
pdf("linear_model_scatter_plot.pdf",12,9)
plot(all_with_CD4N$CD4N_t,backwardModel_CD4_final$fitted.values, type = "n", ylab = "measured square root of the CD4 count ", xlab = "predicted square root of the CD4 count", main = "Predicted vs Measured CD4 counts")
points(all_with_CD4N$CD4N_t[all_with_CD4N$hivstat == 0],backwardModel_CD4_final$fitted.values[all_with_CD4N$hivstat == 0], col = 4)
points(all_with_CD4N$CD4N_t[all_with_CD4N$hivstat == 1],backwardModel_CD4_final$fitted.values[all_with_CD4N$hivstat == 1], col = 2)
dev.off()
```
...
We want to predict the number of people a woman with HIV lives with.
...
```{r}
hoslem.test(all_with_living_with$living_with, fitted(backward_poisson))
```
...
all_with_CD4N %>% 
  ggplot() +
  geom_boxplot(aes(x = as.factor(hivstat), y = backwardModel_CD4_final$fitted.values , color = as.factor(hivstat))) + 
...
all_with_CD4N$ever_sex_with_hiv_male[all_with_CD4N$ever_sex_with_hiv_male == - 1] <- 3
all_with_CD4N$shared_needles[all_with_CD4N$shared_needles == - 1] <- 3
all_with_CD4N$ever_jail[all_with_CD4N$ever_jail == - 1] <- 3
View(all_with_CD4N)

newCD4 <- lm(CD4N_t ~ as.factor(marital_status) + household_income + sex_partners+ as.factor(shared_needles) + as.factor(ever_jail) + as.factor(ever_sex_with_hiv_male) + hivstat, data = all_with_CD4N)
summary(newCD4)
